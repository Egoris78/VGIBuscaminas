...
{ 
    ...
    {
        offset += ((i == 0) ? 0 : desc.attributesList[i-1].numElements * sizeof(float));
        ...
        glVertexAttribPointer(
            ...
            (void*)offset //offset from previous attribute
        );
...
OVertexArrayObject::OVertexArrayObject(const OVertexBufferDesc& vbDesc, const OIndexBufferDesc& ibDesc):OVertexArrayObject(vbDesc)
{
   ...
   // OVertexArrayObject::OVertexArrayObject(vbDesc); // initialize vertex buffer
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, ibDesc.listSize* sizeof(ui32), ibDesc.indicesList, GL_STATIC_DRAW);

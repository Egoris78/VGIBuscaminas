...
OCameraType OCamera::getCamType()
{
	return m_type;
}

f32 OCamera::getFieldOfView()
{
	return m_fov;
}

f32 OCamera::getFarPlane()
{
	return m_farPlane;
}

f32 OCamera::getNearPlane()
{
	return m_nearPlane;
}
...
void OCamera::computeProjectionMatrix()
{
	if (m_type == OCameraType::Perspective)
		...
		m_projection.setOrthoLH((f32)m_screenArea.width * m_fov, (f32)m_screenArea.height * m_fov, m_nearPlane, m_farPlane);
}
